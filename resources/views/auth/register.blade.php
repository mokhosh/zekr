...
<div class="mx-auto h-full flex justify-center items-center bg-gray-300">
    <div class="w-96 bg-blue-900 rounded-lg shadow-xl p-6">
        <h1 class="pb-8 text-white text-2xl">{{ __('auth.register') }}</h1>
            <form method="POST" action="{{ route('register') }}">
                @csrf

                <div class="relative">
                <label for="name" class="uppercase text-blue-500 text-xs font-bold absolute ltr:pl-3 rtl:pr-3 pt-2">{{ __('auth.name') }}</label>
